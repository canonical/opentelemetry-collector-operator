...
def given_total_memory(context):
...
@given(parsers.parse('a user provides "{value}" for the memory_limit_percentage config option'))
...
def then_hard_limit_is(context, clamped_percentage):
    ...
    assert limit_mib >= 0
    assert limit_mib <= TOTAL_MEMORY


@then("the spike limit is 20% of the hard limit")
def then_spike_limit(context):
    limit_mib = context["limit_mib"]
