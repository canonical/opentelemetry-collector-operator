...
from charm import OpentelemetryCollectorOperatorCharm


def test_httpbin_pebble_ready():
    # Arrange:
    ctx = testing.Context(OpentelemetryCollectorOperatorCharm)
...
def test_config_changed_valid_can_connect():
    ...
    # Arrange:
    ctx = testing.Context(OpentelemetryCollectorOperatorCharm)  # The default config will be read from charmcraft.yaml
    container = testing.Container("httpbin", can_connect=True)
