...
"""Feature: COS Agent log_files integration."""
...
def test_cos_agent_log_files_are_scraped(ctx, config_folder, unit_name):
    ...
    # GIVEN a charm with a cos-agent relation that provides log_files
    cos_agent_relation = SubordinateRelation(
        ...
        remote_unit_data={
            "config": json.dumps({
                ...
                "metrics_scrape_jobs": [],
                "log_slots": [],
                "log_files": ["/var/log/app.log", "/var/log/app-error.log"]
            })
        },
    )
